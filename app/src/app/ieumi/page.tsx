...
const IeumiPage = () => {
    ...
    const CURRENT_USER_ID = useRef<string | null>(null);
    ...
    useEffect(() => {
        ...
        const setupChatRoom = async () => {
            try {
                ...
                console.warn("채팅방 없음. 새로 생성합니다.", error);
                try {
                    ...
                    const newRoomId = createResponse.data.data;
    ...
    /** 채팅 내역 불러오기 */
    const loadMessages = async (roomId: string, nextOffset: number) => {
        if (!jwtToken || loadingOldMessages) return;
        setLoadingOldMessages(true);

        try {
            const response = await axios.post(
                "http://localhost:8080/api/chat/room/messages",
                { chatRoomId: roomId, offset: nextOffset },
                { headers: { Authorization: `Bearer ${jwtToken}` } }
            );
            ...
            const newMessages = data.messages.map((msg: any) => ({
                sender:
                    msg.senderId?.toLowerCase() === CURRENT_USER_ID.current?.toLowerCase()
                        ? "user"
                        : "bot",
                text: msg.content,
            }));
            ...
            onConnect: () => {
                ...
                stompClient.subscribe(`/topic/chat/room/${roomId}`, (message) => {
                    ...
                    const formattedMessage: Message = {
                        sender:
                            receivedMessage.senderId?.toLowerCase() ===
                            CURRENT_USER_ID.current?.toLowerCase()
                                ? "user"
                                : "bot",
    ...
    const handleScroll = () => {
        ...
        if (container.scrollTop < 100) {
            loadMessages(chatRoomId!, offset);
        }
    };

    /** 새 메시지 도착 시 하단 스크롤 유지 */
    useEffect(() => {
                ...
                {messages.map((msg, index) =>
                        msg.sender === "bot" ? (
                            <div key={index} className="flex items-start space-x-3">
                                <div className="flex-shrink-0 w-11 h-11 rounded-full bg-orange-100 flex items-center justify-center text-2xl shadow-sm">
                                    <Image
                                        src={logo}
                                        alt="이음이 프로필"
                                        width={44}
